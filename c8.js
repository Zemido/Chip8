...
class C8_CPU {
    ...
    load(program) {
        ...
        this.RAM = new Uint8Array(Array.from(this.RAM.slice(0, 0x200)).concat(Array.from(program).concat(Array.from(new Uint8Array(3584 - program.length)))));
        this.PC = 0x200;
    }
    ...
    step() {
        ...
        switch (opcode >> 12) {
            ...
            case 0xc:
                this.V[x] = (Math.random() * 255) & (byte);
                break;
            // Dxyn: Draw sprite at RAM[I] to RAM[i+nibble] on screen at (VX,VY). Set VF = collision
            case 0xd:
                this.V[0xf] = this.screen.draw(this.V[x], this.V[y], new C8_Sprite(this.RAM.slice(this.I, this.I + nibble))) ? 1 : 0;
                break;
            case 0xe:
                switch (byte) {
                    // Ex9E: Skip next instruction if KEY = VX
                    case 0x9e:
                        if (this.cbKeyboard() === this.V[x])
                            this.PC += 2;
                        break;
